...
from backorder.logger import logging, get_log_file_name
from backorder.exception import backorderException
from threading import Thread
from typing import List

from multiprocessing import Process
from backorder.entity.artifact_entity import ModelPusherArtifact, DataIngestionArtifact, ModelEvaluationArtifact
from backorder.entity.artifact_entity import DataValidationArtifact, DataTransformationArtifact, ModelTrainerArtifact
from backorder.entity.config_entity import DataIngestionConfig, ModelEvaluationConfig
...
class Pipeline(Thread):
    experiment: Experiment = Experiment(*([None] * 11)) #all attribute in experiment initializing with none
    experiment_file_path = None

    def __init__(self, config: Configuartion ) -> None:
        try:
            os.makedirs(config.training_pipeline_config.artifact_dir, exist_ok=True)
            Pipeline.experiment_file_path=os.path.join(config.training_pipeline_config.artifact_dir,EXPERIMENT_DIR_NAME, EXPERIMENT_FILE_NAME)#storing pipeline execution (when it completed)in csv file format 
    ...
    def start_model_trainer(self, data_transformation_artifact: DataTransformationArtifact) -> ModelTrainerArtifact:
        try:
            model_trainer = ModelTrainer(model_trainer_config=self.config.get_model_trainer_config(),  #provided configuration and  artifact entity
